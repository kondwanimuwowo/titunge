...
import type { MarketplaceSeller } from "@/lib/marketplace-db";

export function MakerSpotlight({ sellers }: { sellers: MarketplaceSeller[] }) {
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {sellers.map((seller) => (
            ...
            >
              <ImagePlaceholder shape="circle" className="w-24 h-24" src={seller.avatar} alt={seller.name} />
              <div className="text-lg font-bold mt-2">{seller.name}</div>
              {seller.location && <div className="text-sm text-gray-500">{seller.location}</div>}
              {seller.bio && <div className="text-sm leading-relaxed">{seller.bio}</div>}
