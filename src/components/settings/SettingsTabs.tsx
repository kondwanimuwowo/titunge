...
import { Warehouse, DollarSign, Scissors } from "lucide-react";
...
export default function SettingsTabs({ financialSettings, garmentTypes }: SettingsTabsProps) {
  const [activeTab, setActiveTab] = useState("general");

  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
      <TabsList className="grid grid-cols-3 lg:w-[460px]">
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-foreground">Gloriaz Daughter</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Base Currency</p>
                <p className="text-sm font-medium text-foreground">ZMW (K)</p>
              </div>
            </div>
          </CardContent>
        </Card>
